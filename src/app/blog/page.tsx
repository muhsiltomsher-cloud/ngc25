...
const categories = ['All', 'Guide', 'Case Study', 'News', 'Press'] as const;
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      <Container className="py-12">
        <div className="mb-6 text-center">
          <span className="inline-block rounded-full bg-slate-900/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-slate-600">Blog</span>
          <h1 className="mt-3 text-3xl md:text-4xl font-serif font-medium text-gray-900">Insights & Stories</h1>
          <p className="mt-2 text-gray-600">Guides, case studies and news from NGC.</p>
        </div>
        ...
        <div className="mx-auto mb-8 max-w-2xl">
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search posts (e.g., carpet tiles, acoustic)" className="w-full rounded-full border border-slate-300 bg-white px-5 py-3 text-sm focus:outline-none" />
        </div>

        {/* Hero card */}
        {hero && (
          <Link href={`/blog/${hero.slug}`} className="mb-10 block overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5 focus:outline-none">
            ...
            <div className="p-6 md:p-8">
              ...
              <span className="mt-3 inline-block text-sm font-semibold text-blue-600">Read post →</span>
            </div>
          </Link>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {rest.map(a => (
            <Link key={a.slug} href={`/blog/${a.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm focus:outline-none">
